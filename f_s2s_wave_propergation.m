...
f_us = @(ys,xs)interp2(X,Y,Us,xs,ys,'cubic');
% Calculate the phase angle if the electical field: E = u(x,y)*exp(1i*theta)
f_theta = @(ys,xs,yt,xt)-k/2*((xs.^2+ys.^2)/a+(xt^2+yt^2)/b+k/2/alpha*(xs/a+xt/b).^2+k/2/alpha*(ys/a+yt/b).^2);
% Calculate the integration kernal: E = u(x,y)*exp(1i*theta)
f_kernal = @(ys,xs,yt,xt)f_us(ys,xs).*exp(1i*f_theta(ys,xs,yt,xt));
